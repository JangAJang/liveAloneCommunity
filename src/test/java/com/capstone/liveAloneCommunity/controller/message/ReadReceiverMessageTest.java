...
public class ReadReceiverMessageTest {
    ...
    void readReceiverMessage_Success_ExcludeDeletedMessage() throws Exception{
        // given
        Member sender = getMember("sender");
        Member receiver = getMember("receiver");
        sendMessage(sender, receiver, "senderToReceiver", 5);
        sendMessage(receiver, sender, "receiverToSender", 5);
        deleteMessage(receiver, 1L);
        deleteMessage(receiver, 2L);

        // when // then
        mvc.perform(get("/api/message/receiver?page=0&size=10&readMessageType=RECEIVER")
                        .header("Authorization", getAccessTokenAfterLogIn("receiver"))
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.code").value(200))
