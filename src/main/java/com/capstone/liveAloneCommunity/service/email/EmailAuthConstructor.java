...
public class EmailAuthConstructor {
    ...
    private char getAlphabetInCase(final EmailAuthComponentNumber emailAuthComponentNumber, final Random random) {
        return (char) (random.nextInt(ALPHABET_VARIES.getValue())+emailAuthComponentNumber.getValue());
    }

    private char getNumber(final Random random) {
        return (char) (random.nextInt(NUMBER_RANGE.getValue())+NUMBER_STARTER.getValue());
    }

    private boolean isUpperCase(final int index) {
        return index == UPPER_CASE_STARTER.getValue();
    }

    private boolean isLowerCase(final int index) {
